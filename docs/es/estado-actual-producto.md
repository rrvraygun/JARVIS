...
- GitHub Agent se puede seleccionar en Agents y Conversation. Sus herramientas iniciales
  ...
  remotas se pueden inspeccionar en solo lectura; las escrituras siguen siendo
  planes hasta comprobar de forma independiente el conector acotado configurado
...
La visibilidad de herramientas MCP queda vinculada al scope del especialista
seleccionado. El servidor filtra `tools/list` a sus herramientas MCP registradas
y metadatos compartidos. Durante preflight el catálogo es visible para mantener
la coherencia del esquema, pero ninguna llamada es ejecutable hasta la admisión
tipada. El contrato personalizado se envía una vez por digest e hilo y luego se
usa una referencia de continuidad.
